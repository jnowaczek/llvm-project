...
public:
  ...
  virtual StringRef getPassName() const {
    return StringRef("MBlaze DAG->DAG Pattern Instruction Selection");
  }
private:
...
SelectAddrRegImm(SDValue N, SDValue &Base, SDValue &Disp) {
  ...
  if (N.getOpcode() == ISD::ADD || N.getOpcode() == ISD::OR) {
    int32_t imm = 0;
    if (isIntS32Immediate(N.getOperand(1), imm)) {
      Disp = CurDAG->getTargetConstant(imm, SDLoc(N), MVT::i32);
    ...
    Disp = CurDAG->getTargetConstant(Imm, SDLoc(N), CN->getValueType(0));
    Base = CurDAG->getRegister(MBlaze::R0, CN->getValueType(0));
    return true;
  }

  Disp = CurDAG->getTargetConstant(0, SDLoc(N), TM.getTargetLowering()->getPointerTy());
